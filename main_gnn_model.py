"""
Fused Bi-Level FraudGNN — the flagship model (architecture.md section 3).

BatchNet     : offline, full-graph GCNConv pathway. Captures global
               structural fraud signals. Only usable when the full graph is
               available (Colab training / offline batch scoring — not
               production real-time inference).
RealTimeNet  : inductive SAGEConv pathway. Learns aggregation functions, not
               fixed embeddings, so the SAME trained weights generalize to
               nodes/subgraphs never seen during training. This is what runs
               alone in production.
LambdaNeuralNetwork : fuses the two embeddings via concatenation + linear
               projection. use_batch=True (training, offline batch scoring)
               uses both pathways; use_batch=False (real-time inference)
               zeroes the BatchNet half, so RealTimeNet's weights work
               standalone without ever needing the full graph.
FraudGNN     : LambdaNeuralNetwork + classifier head, outputs a fraud
               probability in [0, 1] per node (sigmoid, not 2-class logits).
"""
